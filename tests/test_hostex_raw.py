...
def test_api_get_does_not_forward_the_token_across_origins(monkeypatch):
    """A redirect off api.hostex.io is refused, not followed.

    urllib copies a Request's headers onto the redirected request without
    checking the host, so following one would hand HOSTEX_TOKEN to whatever it
    points at — confirmed before the fix by watching it arrive at a second
    origin. Refusing is stricter than stripping the header: the request stops
    here rather than failing authentication somewhere we did not intend to
    talk to. `bin/hostex-poll.py` set this precedent.
    ...
    code calls what we already decided to call.
    """
    import http.server
    ...
    origin_port, elsewhere_port = free_port(), free_port()

    class Redirecting(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
            self.send_response(302)
            self.send_header("Location", f"http://127.0.0.1:{elsewhere_port}/landed")
            self.end_headers()
    ...
    servers = []
    for port, handler in ((origin_port, Redirecting), (elsewhere_port, Elsewhere)):
    ...
    try:
        mod = load()
        monkeypatch.setattr(mod, "BASE", f"http://127.0.0.1:{origin_port}")
        with pytest.raises(Exception) as caught:
            mod.api_get("/conversations", "the-token-value")
    finally:
        for srv in servers:
            srv.shutdown()

    assert "refusing redirect" in str(caught.value), (
        f"expected the redirect to be refused, got {caught.value!r}"
    )
    assert not received, (
        f"the redirect target was reached at all, carrying {sorted(received)}"
    )
