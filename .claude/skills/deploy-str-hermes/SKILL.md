...
| Files in the home volume owned by `1000`, or by another account | the home was migrated off a bind mount, where it carried the host account's ids | the image runs the agent as `10000:10000` and no longer takes the ids from the invoking account, so re-own it from inside: `docker compose exec -u root hermes chown -R 10000:10000 /var/lib/hermes`, then `just restart`. Scope it to the home and never to `/var/lib/hermes/repo/vault` — that is a host bind, and a recursive chown through it re-owns the operator's own vault checkout |
